...
public class JunitTafelTest extends TestCase {
	...
	public void testPlaatsTegel() {
		...
		// testen van plaatsen van starttegels, en de tegels daarrond
		assertTrue("Plaatsing tegel faalt", tafel.plaatsTegel(tegels.get(0)
				.getTegelString(), coords.get(0)));
		Vector2D coord;
		for (int i = 1; i < 5; ++i) {
			coord = coords.get(i);
			assertTrue("Plaatsing tegel " + (i + 1) + " faalt. coord: ("
					+ coord.getX() + ", " + coord.getY() + ")", tafel
					.plaatsTegel(tegels.get(i).getTegelString(), coord));
		}
		assertFalse("Plaatsing zelfde tegel faalt", tafel.plaatsTegel(tegels
				.get(0).getTegelString(), coords.get(0)));
		...
		for (int i = 0; i < 5; ++i) {
			...
			if (t != null) {
				assertTrue("Verkeerde tegel opslag: tegel " + i, t
						.equals(tegels.get(i)));
			}
		}
	...
	public void testIsTegelPlaatsingGeldig() {
		...
		assertFalse(tafel.isTegelPlaatsingGeldig(
				tegels.get(0).getTegelString(), coords.get(0)));

		// in het wilde weg zetten mag niet (tegels moeten aansluiten)
		assertFalse(tafel.isTegelPlaatsingGeldig(
				tegels.get(0).getTegelString(), new Vector2D(10, 10)));

		// landsdelen komen niet overeen
		assertFalse(tafel.isTegelPlaatsingGeldig(
				tegels.get(1).getTegelString(), coords.get(2)));

		// en natuurlijk nog een juiste plaatsing testen
		assertTrue(tafel.isTegelPlaatsingGeldig(tegels.get(1).getTegelString(),
				coords.get(1)));
	}
